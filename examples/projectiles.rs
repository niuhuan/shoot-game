//! Projectile style gallery (bullets + missiles).
//!
//! Run: `cargo run --example projectiles`
            ...
            DefaultPlugins
                .set(WindowPlugin {
                    primary_window: Some(Window {
                        title: "Projectiles Gallery".to_string(),
...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    let gallery: [(&str, Vec3, GeometryBlueprint); 5] = [
        (
            "Player Bullet (Raiden needle)",
            Vec3::new(-380.0, 140.0, 0.0),
            GeometryBlueprint::default_bullet(),
        ),
        (
            "Enemy Bullet: Shard",
            Vec3::new(-120.0, 140.0, 0.0),
